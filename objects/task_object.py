...
class task(object):
    ...
    def get_requested_computing_resources_at_vehicle(self) -> float:
        return self._cqu_cycles * cover_MB_to_bit(self._input_data_size) / (self._deadline / 1.1)
    
    def get_requested_computing_resources_at_local_edge(self) -> float:
        return self._cqu_cycles * cover_MB_to_bit(self._input_data_size) / (self._deadline / 6)
